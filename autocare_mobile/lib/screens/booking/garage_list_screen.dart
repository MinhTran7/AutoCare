...
class _GarageListScreenState extends State<GarageListScreen> {
  ...
  List<Map<String, dynamic>> get _selectedServices =>
      List<Map<String, dynamic>>.from(widget.draft['services']);
  ...
  Future<void> _loadGarages() async {
    try {
      final serviceIds = _selectedServices.map<int>((s) => s['id'] as int).toList();
      final data = await _bookingService.getGarages(serviceIds: serviceIds, lat: 21.0278, lng: 105.8342);
      if (!mounted) return;
      setState(() {
        _garages = data;
        _isLoading = false;
  ...
  void _continue() {
    ...
    final newDraft = Map<String, dynamic>.from(widget.draft)..['garage'] = garage;
    Navigator.push(context, MaterialPageRoute(builder: (_) => DateTimeScreen(draft: newDraft)));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLoading
        ...
        child: Center(
          child: Text(
            'Khong co garage nao ho tro du cac dich vu ban da chon.\n'
                'Vui long quay lai va chon it dich vu hon.',
                  ...
                  child: ListTile(
                    ...
                    subtitle: Text(
                      [
                        ...
                        if (g['distanceKm'] != null) '${g['distanceKm']} km',
                      ].join(' • '),
                    ),
                    trailing: Icon(
